...
if (!isset($_SESSION['username'])  || $_SESSION['role'] !== 'external') {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['rate_project'])) {
    ...
    $external_id = $_SESSION['username']; // Using username as external identifier
    
    // Ensure rating is between 1 and 5
    $rating = max(1, min(5, $rating));
    
    // Check if external has already rated this project
    $check_query = "SELECT id FROM external_ratings WHERE project_id = ? AND external_id = ?";
    $check_stmt = $conn->prepare($check_query);
    $check_stmt->bind_param("is", $project_id, $external_id);
    $check_stmt->execute();
    $check_stmt->store_result();
    
    if ($check_stmt->num_rows > 0) {
        // Update existing rating
        $update_query = "UPDATE external_ratings SET rating = ?, rated_at = NOW() 
                         WHERE project_id = ? AND external_id = ?";
        $stmt = $conn->prepare($update_query);
        $stmt->bind_param("iis", $rating, $project_id, $external_id);
    } else {
        // Insert new rating
        $insert_query = "INSERT INTO external_ratings (project_id, external_id, rating, rated_at) 
                         VALUES (?, ?, ?, NOW())";
        $stmt = $conn->prepare($insert_query);
        $stmt->bind_param("isi", $project_id, $external_id, $rating);
    }
    
    if ($stmt->execute()) {
        // Update the combined rating in projects table
...
$avg_stmt->bind_param("iiii", $project_id, $project_id, $project_id, $project_id);
    ...
    header("Location: external.php?id=" . $project_id);
    exit();
}
    ...
    <div class="wrapper d-flex">
        ...
        <div class="sidebar p-3">
            ...
            <ul class="nav flex-column">
                <li class="nav-item">
                    <a class="nav-link active" href="external.php">
...
?>">
    <?= htmlspecialchars($project['status'] ?? 'Unknown') ?>
                                    ...
                                    <div class="d-flex justify-content-between align-items-center mt-3">
                                        ...
                                        <a href="external.php?id=<?= $project['id'] ?>" class="btn btn-sm btn-primary">
    View Details
</a>
                                    </div>
                                </div>
